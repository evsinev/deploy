...
| GET | `DASHBOARD_PATH` | Live [dashboard](../guides/dashboard/) HTML shell. |
...
| GET | `/metrics` (exact) | Prometheus exposition. See [Metrics](./metrics/). |
| GET | `/?command=listAgents` | List connected agents. |
| GET | `/?command=run&alias=<alias …>` | Parse and run an [alias](../guides/writing-aliases/) asynchronously. |
